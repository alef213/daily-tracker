# Daily Tracker — Build Plan

## Project Overview

A minimal-zen daily habit and routine tracker web app. Designed for personal use, deployed as a Progressive Web App so it can be added to a phone's home screen. Data persists locally via `localStorage`.
...
**Deployment target:** Vercel (free tier), connected to GitHub repo for auto-deploy.

---

## Concept

The app has two integrated layers on a single daily view:

1. **Daily Routine** — pre-built (but editable) routine items the user moves through each day (e.g., morning anchor, focus sprint, idea capture, emotional check-in, intentional rest). Some have optional journal prompts attached.
2. **My Habits** — user-defined custom habits that persist day-to-day. Each gets checked off daily.

Both layers track:
- Daily completion (checkboxes)
- Streaks (consecutive days complete)
- 30-day completion heat grids
- Active streaks ranking

Plus a **weekly reflection** (3-prompt journal: what worked / what didn't / what to adjust) that surfaces on Sundays but is always accessible.

**Design vibe:** Minimal zen. Warm stone neutrals, generous whitespace, no harsh accents except a small amber flame for streaks. Light mode only.
...
(Run inside the existing `daily-tracker` folder. The `.` targets the current directory.)
...
Replace `index.html` with PWA-friendly meta tags:
...
### 6. Replace src/App.jsx

Use the full component code in the **Application Code** section below.
...
Should serve at `http://localhost:5173/`. Test:
- Add a custom habit, check it off
- Check off a routine item
- Open a journal prompt, write something, save
- Refresh the page — data should persist
- Navigate to "Yesterday" via the left arrow, check off something there
- Open the Settings (gear icon) and edit a routine item
- Open the Journal (book icon) to see past entries
...
**Header**
- Date selector with left/right chevrons; left goes back a day, right disabled when on today
- "Jump to today" button appears when viewing past day
- "Today / Yesterday / [weekday] / [Month Day]" label depending on offset
- Book icon → past entries view
- Gear icon → routine settings view
- Completion count (e.g., "3/8")

**Daily Routine section**
- Defaults to 5 items: Morning Anchor, Focus Sprint, Idea Capture, Emotional Check-in, Intentional Rest
- Each has icon, title, subtitle, optional journal prompt
- Checkbox toggles completion (for selected date)
- Flame icon + streak number when streak > 0
- Book icon on items with `hasJournal: true`; tap opens journal modal
- "Edit" link goes to routine settings

**My Habits section**
- `+` button adds custom habit
- Inline input with Enter-to-save
- Each habit shows title, checkbox, streak, delete button (hover)
- Empty state: "Add a habit to begin"

**Weekly reflection**
- Sunday banner (amber-tinted) at top if not yet completed this week
- Once done, banner becomes "Complete · tap to view"
- Manual access button below habits any other day
- Modal with 3 textareas: worked / didn't / adjust
- Saved keyed by week (Sunday start)

**Sidebar (right column on desktop)**
- 30-day heat grid for every routine item + custom habit (30 small squares, filled = done, with % rate)
- Today's overall completion percentage with progress bar
- Top 5 active streaks ranked

### Journal entry modal
- Shows the routine item's title and journal prompt
- Textarea for the entry
- Save persists to `journal_entries[date][itemId]`

### Past entries view
- Back button to return
- Weekly reflections section (newest first)
- Daily entries section grouped by date (newest first)
- Empty state if no entries

### Routine settings view
- Lists all routine items
- Each row: up/down reorder arrows, edit button, delete button
- "+ Add" button creates a new item
- Edit modal: title, subtitle, icon picker (8 options), toggle journal prompt, journal prompt text
- "Reset to defaults" link at bottom

### Date navigation
- Selected date affects ALL completion toggles and the journal prompt's target date
- Streak calculations always reference actual today (so checking off "yesterday" properly extends streaks)
...
All values JSON-stringified.

```
habits                  → [{ id, name, createdAt }]
routine_items           → [{ id, iconName, title, subtitle, hasJournal, journalPrompt }]
routine_completions     → { "YYYY-MM-DD": { itemId: boolean } }
habit_completions       → { "YYYY-MM-DD": { habitId: boolean } }
journal_entries         → { "YYYY-MM-DD": { itemId: "text" } }
weekly_reflections      → { "YYYY-MM-DD" (Sunday): { worked, didnt, adjust } }
```
...
git init
git add .
git commit -m "Initial commit: daily tracker MVP"
git branch -M main
...
3. Import the `daily-tracker` repo
4. Accept all defaults (Vite is auto-detected)
5. Click Deploy
6. Within ~30 seconds, you'll have a URL like `daily-tracker-xyz.vercel.app`
...
1. **localStorage is per-device.** Data does not sync between phone and desktop. Clearing browser data wipes everything.
2. **No backup/export.** Users cannot currently export their journal entries or completion data. Future enhancement: add an export-to-JSON button in settings.
3. **No PWA service worker.** App requires network to load the initial bundle. Once loaded, it works offline (since all state is local), but a first-visit-offline scenario would fail. Future enhancement: add `vite-plugin-pwa` for full offline support.
4. **No timezone handling.** Dates are computed from the user's local browser time. Traveling across timezones could cause minor inconsistencies in streak calculation.

---

## Future Enhancements (not in MVP)

- Calendar picker for jumping to arbitrary past dates
- Export/import data as JSON
- Optional Supabase backend for cross-device sync
- PWA offline support via service worker
- Dark mode toggle
- Notifications/reminders (would require a backend)
- Habit categories or tags
- Notes field on individual day completions